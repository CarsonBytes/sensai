	...
	<div class="col-sm-12">
		<h3><?php echo JText::_('J2STORE_RELATED_PRODUCTS_CROSS_SELLS'); ?></h3>
				...
				<?php foreach($this->cross_sells as $cross_sell_product):?>
					...
					<div class="col-sm-<?php echo round((12 / $columns));?> crossell-item">
					<div class="pro-icon">
						...
						<div class="mod_pic">
							<a href="<?php echo $cross_sell_product->thumb_image;?>" class="modal" style="display:inline; position:relative;">
						...
						<?php
	      				...
	      				?>
		   				<?php if(isset($thumb_image) &&  JFile::exists(JPATH::clean(JPATH_SITE.'/'.$thumb_image))):?>
		   					<img alt="<?php echo $cross_sell_product->product_name ;?>" class="img-responsive j2store-product-thumb-image-<?php echo $cross_sell_product->j2store_product_id; ?>"  src="<?php echo JUri::root().JPath::clean($thumb_image);?>" />
					   	<?php endif; ?>
						...
						<?php if(count($cross_sell_product->options) || $cross_sell_product->product_type == 'variable'): ?>
							<a class="<?php echo $this->params->get('choosebtn_class', 'btn btn-success'); ?>"
								...
								<?php // echo JText::_('J2STORE_CART_CHOOSE_OPTIONS'); ?><i class="fa fa-list-alt" title="View Option"></i> View Option
							</a>
						<?php else: ?>
						<?php
							$this->singleton_product = $cross_sell_product;
