...
abstract class ModJ2StoreProductsHelper
{
	public static function getList(&$params)
	{
		...
		$introtext_limit = $params->get('introtext_count', 100);
		...
		foreach ($items as $item)
		{
			...
			if( isset($product) && $product->enabled && $product->visibility && !in_array($product->j2store_product_id ,$already_exist) ){
				...
				if($params->get('introtext_count',100) > 0 ) {
					$item->displayIntrotext = substr(self::truncate($item->introtext, $introtext_limit) , $introtext_limit);
				} else {
	...
	public static function getImage($item, $jparams) {
		...
		}elseif($getImage == 'j2store_thumb'){
			$image = isset($item->thumb_image) ? $item->thumb_image : "";
		}
		return $image;
	}
